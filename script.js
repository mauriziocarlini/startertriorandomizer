...
function displayTrio(trio) {
    // Crea elementi vuoti *prima* di verificare se il trio è valido
    for (let i = 0; i < 3; i++) {
    ...
    pokemonContainer.innerHTML = ''; // Pulisci il contenitore *dopo* aver aggiunto i div vuoti
...
function choosePokemon(pokemon, trio) {
    chosenPokemon = pokemon; // Aggiorna chosenPokemon con il nuovo Pokémon
    ...
    pokemonDivs.forEach(pokemonDiv => {
        ...
        if (pokemonName === chosenPokemon.name) {
            ...
            pokemonDiv.classList.remove('not-chosen'); // Rimuovi 'not-chosen'
        } else {
            pokemonDiv.classList.remove('chosen'); // Rimuovi 'chosen' dagli altri
            pokemonDiv.classList.add('not-chosen'); // Aggiungi 'not-chosen'
        }
    });
}
